...
import { recordStreamOutcome } from '../reputationTracker.js';
...
export function setReadyCacheEntry(cacheKey: string, data: StreamData, indexerName?: string): void {
  const now = Date.now();
  const isNew = !readyCache.has(cacheKey);
  readyCache.set(cacheKey, { data, indexerName, createdAt: now, expiresAt: now + getReadyTTLMs() });
  // Reputation: a stream reached ready state — record success once per cache entry.
  // cacheKey is `${normalizedUrl}::${title}`, so the title is everything after '::'.
  if (isNew) {
    const sepIdx = cacheKey.indexOf('::');
    const title = sepIdx >= 0 ? cacheKey.substring(sepIdx + 2) : '';
    if (title) { try { recordStreamOutcome(title, indexerName ?? null, true); } catch { /* noop */ } }
  }
  saveCacheToDisk();
}
...
export function setDeadNzbEntry(
  ...
  try { recordStreamOutcome(title, indexerName ?? null, false, error?.message); } catch { /* noop */ }
  saveCacheToDisk();
}
...
export async function getOrCreateStream(
  ...
  // Keep the pending entry authoritative until the preparation itself settles.
