...
import { classifyNzbFiles } from './nzbParser.js';
...
export async function performHealthCheck(
  ...
  try {
    ...
    // Classification is shared with the t=get inspector (see classifyNzbFiles)
    // so the two paths cannot drift apart.
    const classified = classifyNzbFiles(files);
    let containerType: string | undefined = classified.containerType;
    const videoFiles = files.filter(f => isVideoFile(f.subject));
    const archiveFiles = files.filter(f => isCompressedArchive(f.subject));

    log(`${files.length} files (${classified.videoCount} video, ${classified.archiveCount} archive${classified.discImageCount > 0 ? `, ${classified.discImageCount} disc-image` : ''})${containerType ? ` [${containerType}]` : ''}`);
